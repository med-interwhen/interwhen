...
_SAFETY_VIOLATION_MARKERS = ("SAFETY VIOLATION", "SAFETY RULE VIOLATION")
...
def _strip_blocked_calls(messages):
    ...
    result = []
    for msg in messages:
        # Strip safety constraint user messages, violation feedback, and prompt-check warnings
...
def run_each_file(filename, filepath, formatted_timestamp, model_name, eval_model, tokenizer, device, eval_model_base, label_type, batch_size, target_model_name):
    ...
    if len(to_eval_cases) != 0:
        ...
        for example in tqdm(to_eval_cases, desc=model_name):   
            ...
            # Persist the actually-scored (stripped) trajectory so it can be
            # inspected later. This mirrors what format_input_dialog feeds to
            # the shield via _strip_blocked_calls.
