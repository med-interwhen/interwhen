...
def save_results_csv(results: list, output_path: str):
    ...
    with open(output_path, "w", newline="") as f:
        writer = csv.writer(f)
        writer.writerow([
            "idx", "data_id", "num_attempts", "verifier_passed", "compiles",
...
if __name__ == "__main__":
    ...
    logger.info(f"Running on {N} examples with k_runs={args.k_runs}...")

    def process_example(idx):
        data = dataset[int(idx)]
        example_log_dir = os.path.join(reason_dir, f"example_{idx}_{data.data_id}")
        os.makedirs(example_log_dir, exist_ok=True)

        prompt = build_full_prompt(data)
        task_data = {
            "data_id": data.data_id, "description": data.description,
            "signature": data.signature, "lean_data": data.lean_data,
            "spec_desc": data.spec_desc, "tests": data.tests,
            "reject_inputs": data.reject_inputs, "metadata": data.metadata,
        }
        ...
        def run_attempt(j):
            ...
            monitor_final_answer = StepVerifierVerinaSpecMonitor(
                    name="VerinaSpecStepVerifier", task_data=task_data,
                    llm_server=attempt_llm, prompt=prompt,
                    k_steps=args.k_steps, compile_timeout=120,
                )
            ...
            # Run generation, redirect verbose output to log
            old_stdout, old_stderr = sys.stdout, sys.stderr
            sys.stdout = attempt_log
            sys.stderr = attempt_log
            try:
                answer = asyncio.run(stream_completion(
                    prompt, prev_text="", llm_server=attempt_llm,
                    monitors=monitors, add_delay=False,
                    num_calls_index=0, async_execution=True,
                ))
                attempt_log.write(f"\nANSWER:\n{answer}\n")
            except Exception as e:
                attempt_log.write(f"\nERROR: {e}\n")
            ...
            gen_spec = extract_spec_from_response(answer)
            v_passed = False

            # Verifier check: verify_final_spec + sync_verify_compilation
            old_stdout, old_stderr = sys.stdout, sys.stderr
            sys.stdout = attempt_log
            sys.stderr = attempt_log
            try:
                if gen_spec.get("postcond") != "":
                    final_spec, final_compiles, _, _ = asyncio.run(
                    ...
                    gen_spec = final_spec
                    v_passed, _ = monitor_final_answer.sync_verify_compilation(gen_spec)
                else:
                    v_passed = False
            finally:
                sys.stdout, sys.stderr = old_stdout, old_stderr
            ...
            save_reasoning_trace(int(idx), f"{data.data_id}_attempt{j}", prompt + answer, example_log_dir)

            return {"j": j, "output": gen_spec, "verifier_passed": v_passed}

        # Run all k attempts in parallel
        if args.k_runs > 1:
            with ThreadPool(processes=args.k_runs) as attempt_pool:
                attempts = list(attempt_pool.imap_unordered(run_attempt, range(args.k_runs)))
        else:
            attempts = [run_attempt(0)]
        attempts.sort(key=lambda a: a["j"])
        num_attempts = len(attempts)

        # Evaluate every attempt's spec so analysis at any k is possible.
        # Per-attempt dict is kept uniform across datasets:
        #   {j, output, verifier_passed, final_correct}
        for a in attempts:
            a_eval = evaluate_generated_spec(
                data, a["output"], int(idx) * 1000 + a["j"]
            )
            a["final_correct"] = bool(a_eval.get("compiles")) and bool(a_eval.get("full_spec_correct"))
        ...
        generated_spec = chosen["output"]
        verifier_passed = chosen["verifier_passed"]
        eval_result = chosen["eval"]
        final_correct = chosen["final_correct"]

        return {
            "idx": int(idx), "data_id": data.data_id,
    ...
    # Run in parallel across examples, sequential within each
    results = []
    total_correct = 0
    total_examples = 0
    ...
    with ThreadPool(processes=args.processes) as pool:
        for result in tqdm(
            ...
            total_examples += 1
            if result["compiles"] and result["full_spec_correct"]:
                total_correct += 1
    ...
    compile_rate = num_compile / total_examples if total_examples else 0
    ...
    num_full_spec_correct = sum(1 for r in results if r["compiles"] and r.get("full_spec_correct", False))
    
    precond_correct_rate = num_precond_correct / total_examples if total_examples else 0
    postcond_correct_rate = num_postcond_correct / total_examples if total_examples else 0
    spec_sound_rate = num_spec_sound / total_examples if total_examples else 0
    spec_complete_rate = num_spec_complete / total_examples if total_examples else 0
    full_spec_correct_rate = num_full_spec_correct / total_examples if total_examples else 0
    ...
    print(f"\n{'='*60}", file=_real_stderr)
    print(f"FINAL RESULTS - SPECIFICATION GENERATION", file=_real_stderr)
    print(f"{'='*60}", file=_real_stderr)
    ...
    print(f"Compilations: {num_compile} ({compile_rate:.2%})", file=_real_stderr)
    print(f"Precond Soundness: {total_precond_sound_pass}/{total_precond_sound_total} ({precond_sound_rate:.2%})", file=_real_stderr)
    print(f"Precond Completeness: {total_precond_complete_pass}/{total_precond_complete_total} ({precond_complete_rate:.2%})", file=_real_stderr)
    print(f"Postcond Soundness: {total_postcond_sound_pass}/{total_postcond_sound_total} ({postcond_sound_rate:.2%})", file=_real_stderr)
    print(f"Postcond Completeness: {total_postcond_complete_pass}/{total_postcond_complete_total} ({postcond_complete_rate:.2%})", file=_real_stderr)
    print(f"Full Spec Correct: {num_full_spec_correct}/{total_examples} ({full_spec_correct_rate:.2%})", file=_real_stderr)
    print(f"Avg attempts: {summary['avg_attempts']:.2f}", file=_real_stderr)
