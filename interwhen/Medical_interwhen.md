...
| `--line_interval` | 15 | Trigger verification every N non-empty lines |
...
- Non-empty line count reaches `line_interval` (default 15) since the last trigger
...
- `confidence < 0.9` → treat as PASS regardless of label
- `confidence 0.9–0.95` → FAIL, feedback injected without the "your final answer may need to change" directive
- `confidence ≥ 0.95` → FAIL, full directive feedback injected
